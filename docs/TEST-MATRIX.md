...
| Anonymous authorization | Blind RFC 9578 public tokens prove a network/permission/epoch scope without a member ID or enrollment key at redemption | Wrong network, permission, epoch, expiry, key, challenge digest, signature, encoding, membership, and revocation fail closed; replay and double spend rejected | Per-scope epoch keys rotate; independent presentations have distinct token bytes; old tokens expire at the documented epoch bound | `peerless-anonymous-auth` integration suite; scope challenge known answer and every-field mutation matrix |
